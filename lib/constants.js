...
import { CID_TEST_RENT, CID_PET } from "./ids";

// 데모 모드 — 운영: Vercel에 NEXT_PUBLIC_DEMO_MODE=false 설정
// 미설정·true → 시연 통과. false → 실서비스(느슨한 통과·폴백 시크릿 금지).
...
/** 데모 회원사 — 클라 폴백·서버 라우트가 모두 이 목록 하나를 참조 (중복 정의 금지)
 * companyId는 빌트인(1001/2001)만 고정 — lib/ids.js SSOT.
 */
export const DEMO_MEMBERS = [
  { code: "1001", company: "테스트렌탈", service: "렌탈", vertical: "rent", companyId: CID_TEST_RENT },
  { code: "1002", company: "스피드렌탈", service: "렌탈", vertical: "rent" },
  { code: "1003", company: "하나모빌리티", service: "렌탈", vertical: "rent" },
  { code: "2001", company: "해피펫분양", service: "반려 분양", vertical: "pet", companyId: CID_PET },
  { code: "2002", company: "따뜻한구조", service: "구조·입양", vertical: "pet" },
];

/** 데모용 사업자 코드 (서버 조회 실패 시 폴백) — DEMO_MEMBERS에서 파생 */
export const DEMO_PROVIDER_CODES = DEMO_MEMBERS.reduce((acc, m) => {
  (acc[m.vertical] = acc[m.vertical] || []).push({ code: m.code, company: m.company, service: m.service });
  return acc;
}, {});
...
export const CAMPAIGN_HEADLINE = "내가 지켜온 약속이, 다음 믿음으로 이어집니다.";
export const CAMPAIGN_LEAD =
  "내가 지켜온 약속이, 다음 믿음으로 이어집니다.";
