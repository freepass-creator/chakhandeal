// Admin 자격 없을 때 서버 메모리 스토어 (Cursor 로컬 시연용). 프로세스 생존 동안만 유지.
...
export function mockListRisksByKey(matchKey) {
  return g.__rsProStore.risks.filter((r) => r.matchKey === matchKey && r.status === "active");
}

export function mockAddRisk(payload) {
  const row = { id: rid("rk_"), ...payload, createdAt: Date.now() };
  g.__rsProStore.risks.unshift(row);
  return row.id;
}

export function mockAddConsent(payload) {
  const id = rid("cs_");
  g.__rsProStore.consents.unshift({ id, ...payload, createdAt: Date.now(), completedAt: Date.now() });
  return id;
}

export function mockAddAudit(payload) {
  g.__rsProStore.audits.unshift({ id: rid("au_"), ...payload, createdAt: Date.now() });
}

export function mockListAudits(limit = 50) {
  return [...g.__rsProStore.audits].slice(0, limit);
}

export function mockResolveByIdentity(name, birth) {
  const b = cleanBirth(birth);
  const key = makeMatchKey(name, birth);
  g.__rsProStore.risks.forEach((r) => {
...
export function mockListAllRisks() {
  return [...g.__rsProStore.risks].sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
}

export function mockListAppeals() {
  return [...g.__rsProStore.appeals].sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
}

export function mockAddAppeal(payload) {
  const row = { id: rid("ap_"), ...payload, createdAt: Date.now() };
  g.__rsProStore.appeals.unshift(row);
  return row.id;
}

export function mockResolveAppeal(appealId, name, birth) {
  const a = g.__rsProStore.appeals.find((x) => x.id === appealId);
  if (a) a.status = "resolved";
  mockResolveByIdentity(name, birth);
}

export function mockListConsents(company) {
  const all = [...g.__rsProStore.consents].sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
...
export function mockAddMember(payload) {
  ...
  if (g.__rsProStore.members.some((m) => m.email === payload.email || m.id === id)) {
    const err = new Error("이미 가입된 이메일입니다.");
    err.status = 409;
    throw err;
  }
  const row = { id, ...payload, createdAt: Date.now() };
  g.__rsProStore.members.unshift(row);
  return row;
}

export function mockListPendingMembers() {
  return g.__rsProStore.members
    .filter((m) => m.status === "pending")
    .sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
}

export function mockFindMemberByEmail(email) {
  return g.__rsProStore.members.find((m) => m.email === email) || null;
}

export function mockFindMemberByCode(code) {
  return g.__rsProStore.members.find((m) => m.code === code && m.status === "approved") || null;
}

export function mockApproveMember(id, code) {
  const m = g.__rsProStore.members.find((x) => x.id === id);
  if (!m) return null;
  m.status = "approved";
  m.code = code;
