...
function seedRisks() {
  return [
    { id: "rk_seed1", matchKey: makeMatchKey("홍길동", "900715"), name: "홍길동", birth: "900715", type: "unpaid", vertical: "rent", license: "31-44-667788-99", phone: "010-5555-1212", company: "스피드렌탈", reason: "대여료 4개월 미납", status: "active", subjectUserId: UID_HIT, createdAt: Date.now() - 5e8 },
    ...
    { id: "rk_seed4", matchKey: makeMatchKey("김반려", "950101"), name: "김반려", birth: "950101", type: "abandon", vertical: "pet", phone: "010-7777-8888", company: "해피펫분양", reason: "분양 후 유기 신고", status: "active", subjectUserId: UID_PET_HIT, ownerCompanyId: CID_PET, createdAt: Date.now() - 2e8 },
...
export function mockApproveMember(id, code, companyId) {
  ...
  m.status = "approved";
  m.code = code;
  // companyId는 승인 시 새로 발급하거나 기존값 유지. 무작위 code(genCode)에서 파생하지 않는다 —
  // code가 우연히 1001/2001이면 빌트인 companyId를 상속해 두 회사가 동일 companyId를 공유하게 됨.
