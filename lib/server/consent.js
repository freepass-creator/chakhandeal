...
export async function completeConsent(body, actor = "guest", subjectToken) {
  ...
  if (body.photos?.id || body.idImage) {
    idPhoto = await saveDataUrl(`${photoBase}/id.jpg`, body.photos?.id || body.idImage);
  }
  if (body.photos?.face || body.faceImage) {
    facePhoto = await saveDataUrl(`${photoBase}/face.jpg`, body.photos?.face || body.faceImage);
  }
  // 서명 PNG — dataURL이면 저장. 불리언만 오면(레거시) 경로 없음.
  const sigRaw = body.signature || body.signatureDataUrl || "";
  if (typeof sigRaw === "string" && sigRaw.startsWith("data:")) {
    signaturePath = await saveDataUrl(`${photoBase}/sig.png`, sigRaw);
  }
  ...
  const piiRef = await upsertPiiVault({
    ...
    fields: {
      ...
      birth,
      phone,
      idImage: typeof idPhoto === "string" && idPhoto.startsWith("data:") ? idPhoto : "",
      faceImage: typeof facePhoto === "string" && facePhoto.startsWith("data:") ? facePhoto : "",
    },
  });

  const signed = !!(signaturePath || body.signed);
  ...
  const doc = {
    ...
    cert,
    photosRef: {
      id: typeof idPhoto === "string" && idPhoto.startsWith("data:") ? "" : idPhoto,
      face: typeof facePhoto === "string" && facePhoto.startsWith("data:") ? "" : facePhoto,
    },
    piiRef,
