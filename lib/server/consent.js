...
async function resolveOwnerCompanyId(code, company) {
  ...
  if (ready && company) {
    try {
      const snap = await db.collection("members").where("company", "==", company).where("status", "==", "approved").limit(1).get();
      if (!snap.empty) return snap.docs[0].data()?.companyId || "";
    } catch { /* ignore */ }
  }
  return "";
...
export async function completeConsent(body, actor = "guest") {
  ...
  const ownerCompanyId = await resolveOwnerCompanyId(body.code || "", body.company || "");
