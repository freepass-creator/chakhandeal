...
import { createAndStoreContractPdf } from "./contractPdf";

const COL = "contract_instances";
const FILE = "contract_instances";
const DEFAULT_TTL_MS = 14 * 24 * 60 * 60 * 1000;
const SIGNING_LEASE_MS = 2 * 60 * 1000;
...
export function toMemberStatus(inst, { origin = "" } = {}) {
  ...
  return {
    ...
    documentReady: !!inst.documentPath,
    documentSha256: inst.documentSha256 || "",
    documentBytes: Number(inst.documentBytes) || 0,
    documentUrl: inst.documentPath
      ? `${String(origin || "").replace(/\/$/, "")}/api/v1/contract/${encodeURIComponent(inst.contractId)}/document?format=pdf`
      : "",
  };
}
...
/**
 * 서명 완료는 PDF 생성까지 수 초가 걸린다. 그 사이 같은 계약에 두 요청이 들어오면
 * 서로 다른 서명으로 같은 PDF 경로를 덮어쓸 수 있으므로 저장소에서 먼저 임대 잠금을 잡는다.
 */
async function claimSigning(contractId) {
  const token = randomBytes(16).toString("base64url");
  const now = Date.now();
  const { ready, db } = getAdmin();

  if (ready) {
    const ref = db.collection(COL).doc(contractId);
    return db.runTransaction(async (tx) => {
      const snap = await tx.get(ref);
      if (!snap.exists) return null;
      const current = { contractId, ...snap.data() };
      if (current.status === "signed") {
        throw Object.assign(new Error("이미 서명된 계약입니다."), { status: 409, code: "ALREADY_SIGNED" });
      }
      const activeLease = current.status === "signing"
        && Number(current.signingStartedAt || 0) + SIGNING_LEASE_MS > now;
      if (activeLease) {
        throw Object.assign(new Error("서명 완료본을 생성하고 있습니다."), { status: 409, code: "SIGN_IN_PROGRESS" });
      }
      const previousStatus = current.status === "signing"
        ? String(current.signingPreviousStatus || "opened")
        : String(current.status || "opened");
      const claimed = {
        ...current,
        status: "signing",
        signingToken: token,
        signingStartedAt: now,
        signingPreviousStatus: previousStatus,
      };
      const { contractId: _id, ...stored } = claimed;
      tx.set(ref, stored);
      return claimed;
    });
  }
  ...
  const store = fileLoad();
  const current = store.byId[contractId] || null;
  ...
  store.byId[contractId] = claimed;
  fileSave(store);
  return claimed;
}

async function finishSigning(inst, token) {
  const completed = {
    ...inst,
    status: "signed",
    signingToken: "",
    signingStartedAt: null,
    signingPreviousStatus: "",
  };
  const { ready, db } = getAdmin();
  if (ready) {
    const ref = db.collection(COL).doc(inst.contractId);
    await db.runTransaction(async (tx) => {
      const snap = await tx.get(ref);
      if (!snap.exists || snap.data()?.signingToken !== token) {
        throw Object.assign(new Error("서명 처리 잠금이 만료되었습니다."), { status: 409, code: "SIGN_LEASE_LOST" });
      }
      const { contractId: _id, ...stored } = completed;
      tx.set(ref, stored);
    });
    return completed;
  }
  const store = fileLoad();
  if (store.byId[inst.contractId]?.signingToken !== token) {
    throw Object.assign(new Error("서명 처리 잠금이 만료되었습니다."), { status: 409, code: "SIGN_LEASE_LOST" });
  }
  store.byId[inst.contractId] = completed;
  fileSave(store);
  return completed;
}

async function releaseSigning(contractId, token) {
  const { ready, db } = getAdmin();
  if (ready) {
    const ref = db.collection(COL).doc(contractId);
    await db.runTransaction(async (tx) => {
      const snap = await tx.get(ref);
      if (!snap.exists || snap.data()?.signingToken !== token) return;
      tx.set(ref, {
        status: snap.data()?.signingPreviousStatus || "opened",
        signingToken: "",
        signingStartedAt: null,
        signingPreviousStatus: "",
      }, { merge: true });
  ...
  const store = fileLoad();
  const current = store.byId[contractId];
  if (!current || current.signingToken !== token) return;
  store.byId[contractId] = {
    ...current,
    status: current.signingPreviousStatus || "opened",
    signingToken: "",
    signingStartedAt: null,
    signingPreviousStatus: "",
  };
...
export async function completeContractSign(contractId, { signatureDataUrl, subjectUserId = "", matchKey = "", ip = "" }) {
  ...
  inst = await claimSigning(contractId);
  if (!inst) return null;
  const signingToken = inst.signingToken;
  ...
  // Do not expose `signed` to ERP polling until its immutable PDF exists.
    const document = await createAndStoreContractPdf(inst);
    inst.documentPath = document.documentPath;
    inst.documentSha256 = document.documentSha256;
    inst.documentBytes = document.documentBytes;

  // 보관기간(계약 종료 후 5년, 부가가치세법 §32② 증빙)을 서명 시점에 박는다.
  // 넣을 때 정해두지 않으면 나중에 「언제 지워야 하나」를 알 수 없다.
    inst.purgeAt = computePurgeAt(inst, { now });

    return await finishSigning(inst, signingToken);
  } catch (error) {
    await releaseSigning(contractId, signingToken).catch(() => {});
    throw error;
  }
}
