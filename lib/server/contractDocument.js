 ...
 */
export async function renderContractDocument(inst, { templateDir = null } = {}) {
  ...
  const inject = `<script>window.__SEALED__=${safeJson(sealedPayload(inst))};</script>`;
  const head = html.includes("</head>")
    ? html.replace("</head>", `${inject}\n</head>`)
    : `${inject}\n${html}`;

  return head.replace(/<body([^>]*)>/i, (m, attrs) => `<body${attrs}>${chrome(inst)}`);
}
