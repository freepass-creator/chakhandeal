...
// payload는 PII 원문 대신 HMAC 바인딩(nameHash/birthHash/phoneHash)만 포함.
...
/** @returns {{ userId: string, method: string } | null} */
export function verifyIdentityToken(token) {
  ...
  return { userId: payload.userId, method: payload.method || "unknown" };
}
