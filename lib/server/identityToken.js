...
// payload: userId + matchKey. matchKey=HMAC(이름|생년)이 곧 이름+생년 결속 —
// 별도 nameHash는 두지 않는다(검증에서 안 쓰이면 '결속됐다'는 허상이 되므로).
// ⚠ payload는 base64url(JSON) 평문(암호화 아님) — 소지자=본인이라 자기 userId/matchKey는 열람 가능.
...
/**
 * @param {{ userId: string, name: string, birth: string, method?: string }} p
 * @returns {string}
 */
export function issueIdentityToken({ userId, name, birth, method = "unknown" }) {
