...
function summarizeRows(rows, key, actor) {
...
/** matchKey로만 조회 — 본인 경로(토큰-only). 레거시 name 평문 폴백 없음. */
export async function checkRiskByKey(matchKey, { actor = "anonymous" } = {}) {
  ...
  const result = summarizeRows(rows, key, actor);
  await writeAudit({
    action: "check",
    actor,
...
export async function checkRisk({ name, birth, actor = "anonymous" }) {
  ...
  // Phase 2: 레거시 name-평문 폴백 제거 — matchKey만.
