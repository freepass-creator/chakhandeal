...
export function buildContractRecord(inst, { now = Date.now() } = {}) {
  ...
  const a4 = {
     ...
     * ⚠ 값을 `[라벨, 값]` 튜플로 두지 말 것 — **Firestore 는 배열 안의 배열을 저장하지 못한다.**
     *   저장 시 「Property a4 contains an invalid nested entity」로 통째로 거부된다.
     */
    blocks: atoms.sections.map((sec) => ({
      heading: sec.title,
      rows: sec.rows.map((r) => ({ k: r.label, v: r.value })),
      confirmedAt: sec.confirmedAt,
    })),
    inputs: atoms.inputs.map((f) => ({ k: f.label, v: f.value })),
    consents: atoms.consents.map((c) => ({ k: c.label, v: c.agreed ? "동의함" : "동의하지 않음" })),
