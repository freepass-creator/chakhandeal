...
import { CID_TEST_RENT, CID_PET } from "@/lib/ids";

// 운영자 화이트리스트 — 항상 승인됨 (companyId는 lib/ids.js SSOT)
const BUILTIN = {
...
export async function login(email, pw) {
  ...
  // 데모 계정·비밀번호는 서버(/api/v1/auth/issue)만 알고 검증한다. 클라이언트 번들에 비번을 두지 않는다.
  const acc = loadAccounts()[e];
  if (acc && acc.pw === pw) {
    // 이 기기에서 가입한 로컬 계정(사용자 본인 localStorage) — 서버 미발급 시 세션만.
...
export async function approveMember(id) {
  ...
  if (FB_READY) {
    ...
    const existing = snap.exists() ? snap.data()?.companyId : null;
