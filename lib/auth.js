...
const BUILTIN = {
  ...
  "test@test.com":    { company: "테스트렌탈", role: "member", code: "1001", status: "approved" },
};
...
export const TEST_LOGIN = { email: "test@test.com", pw: "test1234" };
...
export async function login(email, pw) {
  ...
  if (acc && acc.pw === pw) {
    ...
    return setSession({ email: e, company: acc.company, role: acc.role || "member", code: acc.code || "", status: acc.status || "approved" });
  }
  if (MOCK_PW[e] && MOCK_PW[e] === pw) {
    const b = BUILTIN[e];
    return setSession({ email: e, company: b.company, role: b.role, code: b.code, status: b.status });
  }
  return null;
}
