...
export async function createSelfConsent({ name, phone = "", company, code = "", vertical = "", verified, signed = false, photos = null, idImage = "", faceImage = "", identityToken = "", subjectUserId = "" }) {
  const r = await fetch("/api/v1/consent", {
    ...
    body: JSON.stringify({
      ...
      subjectUserId: subjectUserId || verified?.userId || "",
    }),
  });
  const j = await r.json().catch(() => ({}));
