...
const DEMO_MEMBERS = [
  ...
  { code: "1002", company: "스피드렌탈", service: "렌탈" },
  { code: "1003", company: "하나모빌리티", service: "렌탈" },
];
...
export async function createSelfConsent({ name, phone = "", company, code = "", verified, signed = false, photos = null, idImage = "", faceImage = "" }) {
  const r = await fetch("/api/v1/consent", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      name, phone, company, code, verified, signed,
...
export async function queryRisk({ name, birth }) {
  const r = await fetch("/api/v1/check", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ name: (name || "").trim(), birth: cleanBirth(birth) }),
  });
  const j = await r.json().catch(() => ({}));
  if (!r.ok || !j.ok) {
