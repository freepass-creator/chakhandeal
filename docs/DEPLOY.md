...
## A-2. ⚠ 리전 — 주민등록번호를 다루므로 국내를 벗어나면 안 된다

계약서 「01 계약자 정보」가 **주민등록번호·운전면허번호·주소**를 요구하고, 착한거래가 이를 받기로 했다
(2026-08-08 결정). 국외로 나가면 개인정보보호법 §28-8(국외이전) 동의가 따로 필요해진다.
**세 곳을 모두 서울로 맞추면 국외이전 자체가 없다.**

| | 무엇이 지나가나 | 설정 | 상태 |
|---|---|---|---|
| Firestore | 계약 데이터 · 주민번호(암호화) | `asia-northeast3` 서울 | ✅ 설정됨 |
| Cloud Storage | 신분증 · 셀피 · 서명 · 제출서류 | `asia-northeast3` 서울 | ✅ 설정됨 |
| **Vercel 함수** | **위 데이터가 «지나가는» 서버** | **`icn1` 서울** | ✅ `vercel.json` |

> **`vercel.json` 의 `regions: ["icn1"]` 을 지우지 말 것.** 지우면 기본값(미국 버지니아 `iad1`)로 배포되고,
> Firestore가 서울이어도 **신분증이 미국 서버를 거쳐** 들어간다. 거치는 것도 이전이다.

**리전은 프로젝트 생성 시 정해지고 나중에 못 바꾼다.** Firebase 프로젝트를 새로 만들 일이 있으면
반드시 `asia-northeast3`(서울)로 만든다.

**리전을 못 고르는 곳이 하나 있다** — Firebase Authentication 은 구글이 미국에서 처리한다.
다만 담기는 것은 이메일·연락처·UID 뿐이고 주민번호는 `pii_vault`(Firestore 서울)에 들어간다.
→ **개인정보 처리방침에 그 사실을 적어 둔다.**
